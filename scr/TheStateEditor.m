...
FO.maxFreq = 60; %default starting frequency extent
...
if FileExistsIn([baseName,'.eegstates.mat'])
    ...
    if supressLoadGUI == 0
        ...
        mOptions = 'None|Load From .whl file (head tracking)|Load from eeg ch(s) (accelerometer/motion pad)|Load from eeg ch(s) (MEG)|Load from .mat file';
...
if supressGUI == 1
    ...
    StateEditorSetup(StateInfo.fspec, StateInfo.motion, [], rawEeg, baseName, FO, eegFS);
end
...
FO.fig = figure('KeyReleaseFcn', {@DefKey}, 'Position', get(0,'Screensize'));
...
switch nCh
    ...
    case 3
        ...
        a = annotation('textbox', 'Units', 'Normalized', 'EdgeColor', 'none');
        set(a, 'String', ['\bf\color{black}\fontsize{11}Ch', int2str(FO.Chs(1))], 'Position', [-0.0050    0.8200    0.1000    0.1000]);

        a = annotation('textbox', 'Units', 'Normalized', 'EdgeColor', 'none');
        set(a, 'String', ['\bf\color{black}\fontsize{11}Ch', int2str(FO.Chs(2))], 'Position', [-0.0050    0.6150    0.1000    0.1000]);

        a = annotation('textbox', 'Units', 'Normalized', 'EdgeColor', 'none');
        set(a, 'String', ['\bf\color{black}\fontsize{11}Ch', int2str(FO.Chs(3))], 'Position', [-0.0050    0.410    0.1000    0.1000]);
...
for i = 1:nCh
    FO.spec{i} = log(convWithIn(spec{i}, hanning(FO.hanningW)));
end
...
info{end + 1} = ['\color[rgb]{', num2str(colors.states{2}), '}2: Drowsy/Light'];
...
if length(FO.stateHistory) > FO.stateHistoryNum;
    ...
    b = msgbox('Loosing a bit of history');
    uiwait(b);
end
...
set(incHist, 'Units', 'normalized', 'Position', [0.1, 0.36, 0.35, 0.1]);
...
if FO.hanningW == newW
    ...
    if newW == 0
        for i = 1:FO.nCh
            FO.spec{i} = log(FO.unsmoothedSpec{i});
        end
    else
        for i = 1:FO.nCh
            FO.spec{i} = log(convWithIn(FO.unsmoothedSpec{i}, hanning(FO.hanningW)));
        end
    end
    for i = 1:FO.nCh
...
function changeYlim(obj,event)
FO = guidata(obj);

for i = 1:FO.nCh
    n1 = get(FO.ylimbox{i}, 'String');
    n2 = str2num(n1);
    yrange = [-n2 n2];
    disp(yrange);
    set(FO.eax{i}, 'YLim', [-n2 n2]);
    axes(FO.eax{i});
end
UpdateText;
updateEEG;
